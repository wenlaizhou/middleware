...
func MustCompile(jpath string) *JsonpathCompiled {
	c, err := JsonpathCompile(jpath)
	if err != nil {
		mLogger.ErrorF("jsonpath.MustCompile 错误: %v", err.Error())
		return nil
	}
	return c
}
...
func JsonpathCompile(jpath string) (*JsonpathCompiled, error) {
	...
	tokens = tokens[1:]
	res := JsonpathCompiled{
		path:  jpath,
		steps: make([]jsonpathStep, len(tokens)),
	}
	for i, jsonpathToken := range tokens {
		op, key, args, err := parse_token(jsonpathToken)
		if err != nil {
			return nil, err
		}
		res.steps[i] = jsonpathStep{op, key, args}
	}
	return &res, nil
}
...
func jsonpathTokenize(query string) ([]string, error) {
	tokens := []string{}
	//	token_start := false
	//	token_end := false
	jsonpathToken := ""
	...
	for idx, x := range query {
		jsonpathToken += string(x)
		// //fmt.Printf("idx: %d, x: %s, jsonpathToken: %s, tokens: %v\n", idx, string(x), jsonpathToken, tokens)
		if idx == 0 {
			if jsonpathToken == "$" || jsonpathToken == "@" {
		...
		if jsonpathToken == "." {
			...
			// fmt.Println("else: ", string(x), jsonpathToken)
			if strings.Contains(jsonpathToken, "[") {
				// fmt.Println(" contains [ ")
				if x == ']' && !strings.HasSuffix(jsonpathToken, "\\]") {
					if jsonpathToken[0] == '.' {
						tokens = append(tokens, jsonpathToken[1:])
					} else {
						tokens = append(tokens, jsonpathToken[:])
					}
					jsonpathToken = ""
				...
				if x == '.' {
					if jsonpathToken[0] == '.' {
						tokens = append(tokens, jsonpathToken[1:len(jsonpathToken)-1])
					} else {
						tokens = append(tokens, jsonpathToken[:len(jsonpathToken)-1])
					}
					jsonpathToken = "."
					continue
				}
	...
	if len(jsonpathToken) > 0 {
		if jsonpathToken[0] == '.' {
			jsonpathToken = jsonpathToken[1:]
			if jsonpathToken != "*" {
				tokens = append(tokens, jsonpathToken[:])
...
func filter_get_from_explicit_path(obj interface{}, path string) (interface{}, error) {
	steps, err := jsonpathTokenize(path)
